...
void MiningPage::minerError(QProcess::ProcessError error)
{
    ...
    {
        reportToList("miner error", ERROR, NULL);
    }
}
...
{
    if (getMiningType() == ClientModel::SoloMining)
        reportToList("Solo stop", ERROR, NULL);
    else
        reportToList("miner exit", ERROR, NULL);
    ui->list->addItem("");
    minerActive = false;
    resetMiningButton();
...
void MiningPage::minerStarted()
{
    if (!minerActive)
        if (getMiningType() == ClientModel::SoloMining)
            reportToList("Solo start", ERROR, NULL);
        else
            reportToList("start", STARTED, NULL);
    minerActive = true;
    resetMiningButton();
...
{
    ui->startButton->setText(minerActive ? "Stop" : "Start");
